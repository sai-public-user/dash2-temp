...
class Table extends Component {
    ...
    render() {
        ...
        const { hasPinnedColumns, hasCheckBox, hasCompare } = this.props;
            ...
            <Fragment>
                ...
                <div style={{ width: `${100 - (pinnedHeaders.length * 18 + pinnedHeaders.length)}%`, overflowX: 'scroll', overflowY: 'hidden', position: 'relative' }}>
                    <Fragment>
                        <CustomTable>
                            <HeaderData>
                                <Header sortedCol={sortedCol} hasCompare={hasCompare} checkBoxChange={this.headerCheckBoxChange} checkBox={headerCheck} onCellClick={this.onCellClick} headers={filteredHeaders} compare={this.compareClicked} hasPinnedColumns={hasPinnedColumns} pinned={pinned} isPinned={this.isPinned} />
                            </HeaderData>
                        </CustomTable>
                    </Fragment>
                    <Fragment>
                        <CustomTable>
                            <Rows style={{ overflowX: 'visible', overflowY: 'scroll' }} ref="tbody" onScroll={() => this.scrolled(this.refs.tbody.scrollTop, 'main')}>
                                {Array.isArray(rows) && rows.map(
                                    (row, i) => <Row checked={checked} hasCheckBox={hasCheckBox} checkBoxChange={this.rowCheckBoxChange} row={row} key={i} headers={filteredHeaders} pinned={pinned} />
                                )}
                            </Rows>
			            </CustomTable>
                    </Fragment>
                </div>
                {hasCompare && (<DialogTable
